...
def project_full_success(planned_reps_detail: list, actual_reps: list, high: int, prior_slow_mode: bool,
    ...
    1) План был РОВНЫЙ (все подходы на одном уровне) — независимо от того, было прямое
       совпадение с планом или перевыполнение (см. историю бага: раньше "чудо" перехватывало
       такие случаи ДО проверки на uniform_plan и давало более слабый результат, чем обычная
       быстрая прогрессия дала бы на точном совпадении — план 7,7,7, факт 7,7,8 давал только
       8,7,7 вместо честных 8,8,8, то есть перевыполнение выглядело как ЗАМЕДЛЕНИЕ по
       сравнению с точным попаданием в план — так быть не должно, перевыполнение при ровном
       плане не может быть хуже точного попадания). ВСЕ подходы растут на climb_step вместе,
       slow_mode сбрасывается на False (перевыполнение — сильный сигнал, возвращает быструю
       скорость даже если до этого действовал медленный режим).

    2) План был НЕРОВНЫЙ (кто-то отстаёт от лидера, обычно из-за предыдущего catch-up) И
       есть "чудо" — какой-то подход показал результат СВЕРХ своего плана. Прогрессия сразу
       возвращается на быструю скорость (slow_mode сбрасывается). Новая цель ВСЕГДА
       переносится на ПЕРВЫЙ (ведущий) подход — он растёт на climb_step от своего СТАРОГО
       ПЛАНА (не от факта — чудо может случиться на любом подходе, а прогрессировать "с
       нагруженного", то есть уставшего, конца сессии нелогично), остальные подходы остаются
       на прежнем уровне плана. Например план 6,6,7 факт 6,6,9 -> новая цель 7,6,7 (не 9,9,9
       и не 6,6,10).

    3) План был НЕРОВНЫЙ и точное совпадение с планом (без "чуда") — осторожная скорость:
       подтягиваем ОДИН отстающий подход (по порядку) на climb_step к уровню ведущего, не
       выше него. slow_mode остаётся включённым — вернуться к быстрой скорости можно только
       через ровный план (случай 1) или "чудо" на неровном плане (случай 2).
